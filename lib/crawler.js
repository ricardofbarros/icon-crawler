...
crawler.__fallbackRequestIcon = function (urlObj, file, cb) {
  ...
  return request({
    ...
    if (res.headers['content-type'] && res.headers['content-type'].toLowerCase() === contentType) {
      // Cache image
      cache(type, util.getIconUrl(urlObj, file));
...
crawler.__getFavicon = function (urlObj, $, cb) {
  ...
  // Cache file
  cache('favicon', util.getIconUrl(urlObj, iconUrl));
...
crawler.__getAppleTouchIcon = function (urlObj, $, cb) {
  ...
  // Cache file
  cache('apple-touch', util.getIconUrl(urlObj, iconUrls.href));
...
crawler.__getSvgIcon = function (urlObj, $, cb) {
  ...
  // Cache file
  cache('svg', util.getIconUrl(urlObj, iconUrls.href));
...
crawler.__getFluidIcon = function (urlObj, $, cb) {
  ...
  // Cache file
  cache('fluidapp', util.getIconUrl(urlObj, iconUrls.href));
...
crawler.__getMsAppIcon = function (urlObj, $, cb) {
  ...
  // Cache file
  //cache('msapp', util.getIconUrl(urlObj, iconUrl[0].href));
