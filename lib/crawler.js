  ...
  if (!iconUrl) {
    ...
    return request({
      ...
      return request({
        ...
        url: url.resolve('http://www', urlObj.host + '/favicon.ico'),
        timeout: 5000
      }, function (err, res) {
        if (err || !(res.request && res.request.href)) {
          return cb(err || new Error('Unexpected answer'));
...
crawler.__getAppleTouchIcon = function (urlObj, $, cb) {
  ...
  if (util.isArrayEmpty(iconUrls)) {
    ...
    if (util.isArrayEmpty(iconUrls)) {
      // 1st apple-touch-icon fallback
      return request({
        // url -> http://example.com/apple-touch-icon.png
        url: url.resolve(url.format(urlObj), 'apple-touch-icon.png'),
        timeout: 5000
      }, function (err, res) {
        if (err || !(res.request && res.request.href)) {
          return cb(err || new Error('Unexpected answer'));
        ...
        // 2nd apple-touch-icon fallback
        return request({
          // url -> http://www.example.com/apple-touch-icon.png
          url: url.resolve('http://www', urlObj.host + '/apple-touch-icon.png'),
          timeout: 5000
        }, function (err, res) {
          if (err || !(res.request && res.request.href)) {
            return cb(err || new Error('Unexpected answer'));
...
crawler.__getAllIcons = function (urlObj, $, cb) {
  ...
  return crawler.__executeCrawlers(methodsMap, function (err, results) {
    if (err) {
      return cb(err);
    }
