...
FileCacheMetadata.getExpired = function (cb) {
  return client.zrangebyscore(zsetKey, '-inf', 0, cb);
};

FileCacheMetadata.removeExpired = function (members, cb) {
  var commandsArray = [];

  members.forEach(function (member) {
    commandsArray.push(['del', 'icon-crawler:' + member]);
  });
